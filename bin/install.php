...
if ($argc === 4 || $argc === 3) {
    ...
    $version = $argv[2];
    $app = (string)@$argv[3];
    echo $installer->install($package, $version, $app);
} else {
...
class Installer
{
    ...
    public function __construct() {
        ...
        $this->packages = array(
            ...
            'codeigniter-faster-develop' => array(
                'site'  => 'bitbucket_private',
                'user'  => 'quannda',
                'repos' => 'codeigniter-faster-develop',
                'name'  => 'QuanNDA faster CI  (quannda)',
                'dir'   => array('core', 'helpers'),
                'msg'   => 'Depend on Modular Extensions - HMVC(https://bitbucket.org/wiredesignz/codeigniter-modular-extensions-hmvc)',
                'example_branch' => 'master',
            ),
        );
    }
    ...
    public function install($package, $version, $app_dir)
    {
        ...
        }elseif ($this->packages[$package]['site'] === 'bitbucket_private') {
            $method = 'downloadFromBitbucketPrivate';
        } else {
            throw new LogicException(
                'Error! no such repos type: ' . $this->packages[$package]['site']
            );
        }
        $this->app_dir = 'application';
        if($app_dir){
    ...
    private function downloadFromGithub($package, $version)
    {
        ...
        if (is_string($dir)) {
            ...
            $dst = realpath(__DIR__ . "/../{$this->app_dir}/$dir");
            return array($src, $dst);
        }

        foreach ($dir as $directory) {
            $src[] = realpath(dirname($filepath) . "/$repos-$version/$pre$directory");
            @mkdir(__DIR__ . "/../{$this->app_dir}/$directory", 1);
            $dst[] = realpath(__DIR__ . "/../{$this->app_dir}/$directory");
        }
        return array($src, $dst);
    }
    ...
    private function downloadFromBitbucketPrivate($package, $version)
    {
        $user = $this->packages[$package]['user'];
        $repos = $this->packages[$package]['repos'];
        $url = "git@bitbucket.org:$user/{$repos}.git";
        $filepath = $this->git_clone($package, $url);

        $dir = $this->packages[$package]['dir'];
        if (is_string($dir)) {
            $src = realpath($filepath . "/$dir");
            $dst = realpath(__DIR__ . "/../{$this->app_dir}/$dir");
            return array($src, $dst);
        }

        foreach ($dir as $directory) {
            $src[] = realpath($filepath . "/$directory");
            @mkdir(__DIR__ . "/../{$this->app_dir}/$directory");
            $dst[] = realpath(__DIR__ . "/../{$this->app_dir}/$directory");
        }
        return array($src, $dst);
    }
    ...
    private function git_clone($package, $url)
    {
        $urls = parse_url($url);
        $filepath = $this->tmp_dir;
        if(is_dir($filepath .'/'. $package)){
            $this->recursiveUnlink($filepath .'/'. $package);
        }
        chdir($filepath);
        $stdout = shell_exec ("git clone ". $url);
        if(strpos($stdout, 'git: command not found')!==false){
            die('Please install git.');
        }
        //$this->recursiveUnlink($filepath .'/'. $package .'/.git');
...
function pr($data, $exit = false, $ob_clean = false, $method = 'pr')
{
    if ($ob_clean) ob_clean();
    $level = debug_backtrace();
    $level = $level[0];
    $file = explode(str_replace('/', DIRECTORY_SEPARATOR, __FILE__), $level['file']);
    echo '<br>==== ' . array_pop($file) . ":" . $level['line'] . "<br>\n";
